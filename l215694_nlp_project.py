...
# Initialize error flag for conditional imports
has_numpy_error = False

# First try to import numpy, as many libraries depend on it
try:
    import numpy as np
    print(f"Successfully imported numpy version: {np.__version__}")
except ImportError as e:
    print(f"Error importing numpy: {e}")
    has_numpy_error = True
except ValueError as e:
    if "numpy.dtype size changed" in str(e):
        print("Detected numpy binary compatibility issue")
        has_numpy_error = True
    else:
        raise

# Import essential NLP libraries with fallbacks
if not has_numpy_error:
    # Try NLTK first as it's more robust
    try:
        import nltk
        ...
        print(f"Warning: Could not import NLTK: {e}")
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                print("Using a simple tokenizer instead")
                nlp = None
    except ImportError as e:
        print(f"Warning: Could not import spaCy: {e}")
        nlp = None
    except ValueError as e:
        if "numpy.dtype size changed" in str(e):
            print("Skipping spaCy due to numpy compatibility issue")
            nlp = None
        else:
            raise
...
try:
    import textstat
    print("Successfully imported textstat")
except ImportError as e:
    print(f"Warning: Could not import textstat: {e}")
...
try:
    ...
    if "numpy.dtype size changed" in str(e):
        print("Skipping torch due to numpy compatibility issue")
    else:
        raise

# Conditional imports for transformer models
if 'torch' in globals():
    try:
        ...
        print(f"Warning: Issue importing transformers: {e}")
    
    # Try sentence transformers as alternative
    try:
    ...
    print("Skipping transformer imports since torch is not available")
    sentence_model = None

# Evaluation metrics - using alternatives to bert-score
try:
    from rouge_score import rouge_scorer
    print("Successfully imported rouge_scorer")
except ImportError as e:
    print(f"Warning: Could not import rouge_scorer: {e}")

# For calculating BLEU score
if 'nltk' in globals():
    try:
        from nltk.translate.bleu_score import sentence_bleu, SmoothingFunction
        print("Successfully imported BLEU score")
    except ImportError as e:
        print(f"Warning: Could not import BLEU score: {e}")
...
# For LangChain and RAG - conditional imports
try:
    from langchain.text_splitter import RecursiveCharacterTextSplitter
    print("Successfully imported LangChain core")
    ...
    try:
        ...
        print("Successfully imported LangChain community modules")
    except ImportError as e:
        print(f"Warning: Issue importing LangChain community modules: {e}")
except ImportError as e:
    print(f"Warning: Issue importing LangChain core: {e}")

# Wikipedia and retrieval
try:
    import wikipedia
    print("Successfully imported Wikipedia")
except ImportError as e:
    print(f"Warning: Could not import Wikipedia: {e}")
...
try:
    ...
    print(f"Warning: Could not import Streamlit: {e}")
...
try:
    ...
    import json
    print("Successfully imported utility libraries")
except ImportError as e:
    print(f"Warning: Could not import utility libraries: {e}")

# Groq API client
try:
    import groq
    print("Successfully imported Groq client")
except ImportError as e:
    print(f"Warning: Could not import Groq client: {e}")

print("Import process completed!")

# Define alternative to bert-score for semantic similarity
def calculate_similarity(sentences1, sentences2):
    """Calculate semantic similarity between texts using sentence embeddings or fallback methods"""
    # First try sentence transformers if available
    if 'sentence_model' in globals() and sentence_model is not None:
        try:
            embeddings1 = sentence_model.encode(sentences1)
            embeddings2 = sentence_model.encode(sentences2)
    ...
    # Try scikit-learn vectorizer if available
    if 'CountVectorizer' in globals():
        try:
            vectorizer = CountVectorizer().fit_transform([sentences1, sentences2])
            vectors = vectorizer.toarray()
            return cosine_similarity([vectors[0]], [vectors[1]])[0][0]
        except Exception as e:
            print(f"Error using CountVectorizer: {e}")
    
    # Most basic fallback - word overlap
    words1 = set(sentences1.lower().split())
    words2 = set(sentences2.lower().split())
